...
`npm run build` always regenerates the search index, syncs tracker and OSADL sidecar data, and then runs the static Next.js build. The build script currently sets `NEXT_PRIVATE_BUILD_WORKER=0` to avoid a Next.js 16 webpack worker hang observed in local and CI builds.
...
The site footer reports the latest data update using the newest timestamp from the license corpus, the OSI review tracker sync, and the OSADL checklist sync, shown inline with the page-view counter.

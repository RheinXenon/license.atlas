...
- **License Texts series filter（Atlas）**：Review detail 的 License Texts 左栏在同一 submission 有多个 `license_texts[].series` 时显示 series filter（`All` / `MG0` / `MG-BY` / `MG-BY-OS` / `MG-BY-SA`）。点击 series 只过滤左栏文本版本，不影响右侧正文/diff 结构；点击 timeline 事件的 `Text` 标签会自动切到对应 series 并选中关联文本。Timeline 事件列表和 License Texts 内容区统一 `max-h-[560px] overflow-auto`，减少 tab 切换导致的视口跳动。
- **License Texts 合规口径（Atlas）**：提交的许可证原文来自公开 OSI review/discussion 记录，仅用于研究和审查追踪；版权仍归原作者或 license steward。Atlas UI 在 License Texts tab 顶部展示该提示，并在每个文本详情中保留 `[source ↗]` 原始邮件链接。不要把这些 pending/rejected 草案文本表述为 OSI-authored content 或 OSI endorsement。

...
- HuggingFace models / data pages: `>= 1,000` likes
...
- **HuggingFace**: gated by local `models.parquet` fingerprint (`size:mtimeMs`); records rank by likes.
...
`--source` / `--sources` limits the crawl stage to one or more sources (`github`, `huggingface`, `kaggle`) while preserving the cached raw sidecar data for skipped sources. Use this when only one source's extraction or ranking logic changed, such as the HuggingFace likes-ranking switch.
...
- `/licenses/apache-2.0` renders tracker + OSADL + showcase together; HuggingFace entries display likes
...
- `npm run update:projects -- --source huggingface --force` refreshed only HuggingFace data, rebuilt **26** showcase records, and verified the detail UI now displays HuggingFace likes
- `npm run update:projects -- --skip-crawl` after adding raw top-item thresholds expanded the rollout to **31** showcase records
